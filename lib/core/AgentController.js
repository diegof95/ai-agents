...
class AgentController {
    ...
    /**
     * Register the given agent in the controller pool.
     * The second parameter stand for the initial state of the agent
     * @param {Agent} agent 
     * @param {Object} state0 
     */
    register(agent, state0) {
        if (this.agents[agent.getID()]) {
            ...
            //TODO convert state0 to an inmutable object
    ...
    */
    ...
    next() {
        if (!this.problem.goalTest(this.data)) {
            ...
            if (this.problem.goalTest(this.data)) {
                ...
                if (this.currentAgentIndex >= keys.length - 1) this.currentAgentIndex = 0;
                else this.currentAgentIndex++;
                return true;
            }
        }
    }
    ...
    /**
     * Virtual life loop. At the end of every step it executed the onTurn call back.
     * It could be used for animations of login
     * Turn management for non interactive problems
     */
    loop() {
        let stop = false;
        while (!stop) {
